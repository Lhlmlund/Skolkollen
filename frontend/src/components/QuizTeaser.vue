<template>
  <div class="qt-box">
    <template v-if="latest">
      <p class="qt-label">Senaste rekommendation</p>
    ...
    <template v-else>
      <p class="qt-program"><strong>Inget quizresultat ännu</strong></p>
      <RouterLink to="/quiz" class="qt-link">Gör quizet</RouterLink>
    </template>
  </div>
</template>
...
function loadLatest() {
  try {
    ...
    latest.value = raw ? JSON.parse(raw) : null
  } catch {
    latest.value = null
  }
}
...
.qt-label { margin: 0 0 4px; font-size: .85rem; opacity: .7; }
